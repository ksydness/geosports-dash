...
export async function fetchGroupInfo(
  ...
  const res = await fetch(`${BASE}/api/groups/${groupCode}?date=${todayStr()}`, {
...
/** Fetch played scores for a specific date. Returns null on any error. */
export async function fetchDayScores(
  ...
  try {
    const res = await fetch(`${BASE}/api/groups/${groupCode}?date=${date}`, {
      headers: authHeaders(sessionToken),
    });
